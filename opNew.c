...
__end = fA;
//	      printf("\nValores de la matriz A en mult_ikj task %d\n", __taskid);
//	      imprimeMat(cA, matA, 4, 5);
#pragma omp parallel for private(r)
	for (int __distribSched = __start; __distribSched < __end; __distribSched += __iter) {
	for (int __distrib = __distribSched; __distrib < __distribSched + 1; __distrib++){
	        for (k=0; k<cA; k++) {
	          r = matA[__distrib][k];
	//	  if ((__distrib < 5) && ( k <5) && (__taskid ==1))
//		printf("matA[%d][%d] = %f taskid %d\n", __distrib, k, r,  __taskid);
	          for (j=0; j<cB; j++)
...
//	      printf("\nValores de la matriz C en mult_ikj task %d\n", __taskid);
//	      imprimeMat(cB, matC, 4, 5);
}
	}
...
if(__taskid == 0){
	    ...
	    //  printf("\nValores de la matriz C_ikj\n");
	    //  imprimeMat(C2, matC_ikj, 4, 3);
	    }
